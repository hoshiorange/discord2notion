...
マルチ Guild 対応に伴い、Drive 上の保存階層には **Guild フォルダ層** が挟まる。
これによって「同じ Google アカウント上で複数の Discord サーバを運用する」場合も、
各サーバの議事録が混ざらず、それぞれ別フォルダに整理される。

`config/guilds/<guildId>.json` の `name` フィールドが安全文字のみで構成されていれば、
**`<name>-<guildId>` 形式**でフォルダが作られる。これによって運用者が一覧で見てすぐに
「どのサーバのフォルダか」分かる。`name` が無い・危険文字を含む場合は `<guildId>` のみで作られる。

```
（Drive 上）
meetingBot/
├─ 仕事-1234567890123456789/   ← name="仕事" + guildId（安全文字なので付与）
│   ├─ 2026-05/
...
│   └─ 2026-06/
│       └─ ...
├─ 個人-9876543210987654321/   ← name="個人" + guildId
│   └─ 2026-05/
│       └─ ...
├─ 5555555555555555555/        ← name 未設定の Guild は guildId のみ
│   └─ 2026-05/
│       └─ ...
...
| --- | --- |
| 1 | `meetingBot`（固定） |
| 2 | `<name>-<guildId>` または `<guildId>` または `default`（[ルールは下記](#guild-フォルダ名のサニタイズ)） |
| 3 | `<YYYY-MM>`（セッション開始の月） |
...
| 安全文字のみ | `<name>-<guildId>` |
| 未設定 / 空 / 空白のみ | `<guildId>` |
| 危険文字を含む | `<guildId>`（warn ログ付きでフォールバック） |

**安全文字（許可）**:

- 英数字 `a-z A-Z 0-9`
- ハイフン `-` / アンダースコア `_` / 空白
- 日本語（ひらがな・カタカナ・漢字含む Unicode `\p{L}` 全般）
...
危険文字でフォールバックされた場合、ログに以下のような警告が出る：

```
[drive] Drive folder name fallback: name="a/b" に使用できない文字が含まれるため "1234567890" のみを使用します
```

`name` を Guild JSON で見直して安全文字のみにすれば次回から `<name>-<guildId>` で作成される。
（既に作成済みのフォルダ名は **自動でリネームされない**。Drive 側で手動リネーム可能）
